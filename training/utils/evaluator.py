...
class Evaluator:
    def __init__(self, config, test_set, tokenizer, split_str, step=None, model=None):
        ...
        self.prompts, self.gts, self.prompt_texts = self.get_prompts()
    ...
    def get_prompts(self):
        ...
        for sample in self.test_set:
            ...
            gts.append(gt)
            prompt_texts.append(prompt_text)  # Store decoded prompt text

        return prompts, gts, prompt_texts
    ...
    def calculate_metrics(self, predictions, gts):
        """Calculate token-level, exact match, and per-function accuracy"""
        token_accuracies = []
        exact_matches = []
        ...
        try:
            from utils.metrics import PerFunctionMetrics, clean_to_parser_format, extract_input_output

            per_func_metrics = PerFunctionMetrics()

            for pred, gt, prompt_text in zip(predictions, gts, self.prompt_texts):
                try:
                    # Extract input and output vectors from prompt
                    input_str, output_str = extract_input_output(prompt_text)
                    ...
                    # Process example
                    per_func_metrics.process_example(pred_line, gt_line)
                except Exception as e:
                    print(f"Warning: Failed to process per-function metrics for example: {e}")
                    continue

            # Get final per-function metrics
            function_metrics = per_func_metrics.get_metrics()
            metrics["per_function"] = function_metrics

            # Get exact match accuracy by sequence length
            length_metrics = per_func_metrics.get_length_metrics()
            metrics["exact_match_by_length"] = length_metrics

        except Exception as e:
            print(f"Warning: Failed to compute per-function metrics: {e}")
            import traceback
            traceback.print_exc()

        # Compute functional correctness metrics
        try:
            from utils.metrics import FunctionalCorrectnessMetrics, clean_to_parser_format, extract_input_output

            k = self.config.get("vector", {}).get("k", 10) if hasattr(self.config, 'get') else getattr(getattr(self.config, 'vector', None), 'k', 10)
            func_correctness = FunctionalCorrectnessMetrics(k=k)
            ...
            for pred, gt, prompt_text in zip(predictions, gts, self.prompt_texts):
                try:
                    ...
                    pred_line = clean_to_parser_format(input_str, output_str, pred)
                    gt_line = clean_to_parser_format(input_str, output_str, gt)
                    func_correctness.process_example(pred_line, gt_line)
                except Exception as e:
                    print(f"Warning: Failed functional correctness for example: {e}")
                    continue

            correctness_metrics = func_correctness.get_metrics()
            metrics["functional_correctness"] = correctness_metrics

            correctness_by_length = func_correctness.get_length_metrics()
            metrics["functional_correctness_by_length"] = correctness_by_length

        except Exception as e:
            print(f"Warning: Failed to compute functional correctness metrics: {e}")
            import traceback
            traceback.print_exc()
    ...
    def evaluate(self):
        ...
        # Print metrics (skip dict metrics in print output)
        print("\nEvaluation Metrics:")
        for metric, value in metrics.items():
            if isinstance(value, dict):
                continue  # Skip dict metrics (per_function, exact_match_by_length)
