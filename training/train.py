...
class LitLLM(L.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        # Log aggregate metrics to wandb/Lightning
        _dict_metrics = {"per_function", "exact_match_by_length", "functional_correctness", "functional_correctness_by_length"}
        for metric_name, value in metrics.items():
            if metric_name not in _dict_metrics:  # Skip dict metrics
                self.log(
        ...
        # Log per-function metrics (grouped by metric type)
        if "per_function" in metrics:
            for func_name, func_metrics in metrics["per_function"].items():
                # Group by metric type for easy cross-function comparison
                self.log(
                    f"ExactMatch/{func_name}",
                    func_metrics["exact_match"],
                    on_epoch=True,
                    sync_dist=True,
                )
                self.log(
                    f"TokenAccuracy/{func_name}",
                    func_metrics["token_accuracy"],
                    on_epoch=True,
                    sync_dist=True,
                )
                self.log(
                    f"SequenceAccuracy/{func_name}",
                    func_metrics["sequence_correct"],
                    on_epoch=True,
                    sync_dist=True,
                )

                # Operation-level accuracy (only for functions that have operations)
                if func_metrics.get("operations_accuracy") is not None:
                    self.log(
                        f"OperationsAccuracy/{func_name}",
                        func_metrics["operations_accuracy"],
                        on_epoch=True,
                        sync_dist=True,
                    )

                # Also log the count for reference
                self.log(
                    f"FunctionCount/{func_name}",
                    func_metrics["count"],
                    on_epoch=True,
                    sync_dist=True,
                )

        # Log exact match accuracy by sequence length
        if "exact_match_by_length" in metrics:
            for length, length_metrics in metrics["exact_match_by_length"].items():
                label = f"{length}_function" if int(length) == 1 else f"{length}_functions"
                self.log(
                    f"ExactMatch_Amount/{label}",
                    length_metrics["accuracy"],
                    on_epoch=True,
                    sync_dist=True,
                )

        # Log functional correctness metrics
        if "functional_correctness" in metrics:
            fc = metrics["functional_correctness"]
            for metric_name, value in fc.items():
                if metric_name != "total":
                    self.log(
                        f"FunctionalCorrectness/{metric_name}",
                        value,
                        on_epoch=True,
                        sync_dist=True,
                    )

        # Log functional correctness by sequence length
        if "functional_correctness_by_length" in metrics:
            for length, length_metrics in metrics["functional_correctness_by_length"].items():
                label = f"{length}_function" if int(length) == 1 else f"{length}_functions"
                self.log(
                    f"ResultCorrectness_Amount/{label}",
                    length_metrics["result_correctness"],
                    on_epoch=True,
                    sync_dist=True,
                )

        if wandb.run is not None and hasattr(evaluator, 'predictions_after_delimiter'):
            from utils.metrics import FunctionalCorrectnessMetrics, clean_to_parser_format, extract_input_output

            k = getattr(getattr(self.cfg, 'vector', None), 'k', 10)
            fc_checker = FunctionalCorrectnessMetrics(k=k)

            # Create example table
            examples_table = wandb.Table(columns=["Prompt", "Prediction", "Ground Truth", "Exact Match", "Valid"])
            ...
            for i in indices:
                ...
                # Compute validity (intermediate steps correct AND result correct)
                valid = False
                try:
                    input_str, output_str = extract_input_output(evaluator.prompt_texts[i])
                    pred_line = clean_to_parser_format(input_str, output_str, pred)
                    gt_line = clean_to_parser_format(input_str, output_str, gt)
                    fc_result = fc_checker.process_example(pred_line, gt_line)
                    valid = fc_result['valid']
                except Exception as e:
                    print(f"Warning: Failed to compute validity for example {i}: {e}")

                examples_table.add_data(prompt, pred, gt, exact_match, valid)

            valid_count = sum(1 for row in examples_table.data if row[4] == True)
            print(f"Logging evaluation_examples table: {len(examples_table.data)} rows, columns={examples_table.columns}, valid={valid_count}/{len(examples_table.data)}")
